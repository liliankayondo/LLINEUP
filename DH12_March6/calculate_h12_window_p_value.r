...
setwd('/home/namulil/lstm_projects/funestus_llineup/notebooks/DH12_permutations/New_DH12')
...
load.and.combine.data <- function(pop, find.peaks =T, calculate.P.values = T){
	...
	pre.filenames <- grep(paste('_', pop, '\\.pre', sep = '')) %>%  ## in pop search for .pre files in h12 files and return matching file
 	                 setNames(., stri_extract_first_regex(., '(2RL|3RL|X)+(?=\\.tsv)'))## extract names out and label them according to chromosome
		 
	post.filenames <- grep(paste(paste('_', pop, '\\.post', sep = '')), h12.filenames, value = T) %>%
...
plot.h12.diff <- function(h12.table, 
	...
	h12.min.y <- min(0, min(h12.table[, ..h12.columns, with = FALSE]))  ## changed to see if axis will come out better with small ymax values
h12.max.y <- max(0.05, max(h12.table[, ..h12.columns, with = FALSE]))## changed to include negative values
	# Create the empty plot.
	plot(c(
		cs[1], 
		ce[3]),
		c(min.y, max.y), xlim = c(cs[1] + gaps/2, ce[3] - gaps/2), type = 'n', bty = 'n', xaxt = 'n', yaxt = 'n', xlab = '', ylab = '', main = plot.title, cex.main = 1.2)
...
for (pop in names(h12.tables))
	plot.h12.diff(h12.tables[[pop]]$post, #post
	              filename = paste(pop, 'peak_filter_plot_post.svg', sep = '_'), ## removed './DH12_permutations/' since I was working inside the same folder as I want my results to be stored 
...
for (pop in names(h12.tables))
	plot.h12.diff(h12.tables[[pop]]$pre, #pre, 
	              filename = paste(pop, 'peak_filter_plot_pre.svg', sep = '_'), ## removed './DH12_permutations/' since I was working inside the same folder as I want my results to be stored 
	              p.thresh = p.threshold,
	              plot.title = pop)
...
for (pop in names(h12.tables))
	plot.h12.diff(h12.tables[[pop]]$diff, # diff
	              filename = paste(pop, 'peak_filter_plot_diff.png', sep = '_'), ## removed './DH12_permutations/' since I was working inside the same folder as I want my results to be stored 
	              p.thresh = p.threshold,
	              plot.title = pop)
